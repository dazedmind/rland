      ...
      {careers.length > 0 ? (
        <>
          {careers.map((career: Career) => (
            <div
              key={career.id}
              className="group flex flex-col md:flex-row md:items-center justify-between gap-6 p-6 border border-neutral-200 rounded-xl bg-white transition-all duration-300  hover:bg-slate-50/30"
            >
              <div className="flex-1 space-y-2">

                <div className="text-[10px] font-semibold uppercase tracking-wider bg-primary/10 text-primary px-2 py-1 rounded-full w-fit" >
                  {getDepartmentLabel(career.department)}
                </div>

                <h3 className="text-xl lg:text-2xl font-bold text-slate-900 tracking-tight capitalize">
                  {career.position}
                </h3>

                {/* Metadata */}
                <div className="flex flex-wrap items-center gap-y-2 gap-x-2 text-sm text-slate-500 font-medium">
          ...
          {isPaginated && totalPages > 1 && (
            <div className="flex flex-col sm:flex-row items-center justify-between gap-6 pt-8 mt-4 border-t border-neutral-100">
              <p className="text-sm font-medium text-slate-400">
                Showing <span className="text-slate-900">{page}</span> of{" "}
                <span className="text-slate-900">{totalPages}</span> pages
              </p>
        ...
        <div className="flex flex-col items-center justify-center py-20 px-4 border border-dashed border-neutral-200 rounded-2xl bg-slate-50/50">
          ...
          <p className="text-slate-500 font-medium text-center">
            {total === 0
              ? "No career openings at the moment."
